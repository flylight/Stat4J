...
public class Stat4J {
    ...
    public StringBuilder getStatSource() {
        ...
        records.forEach((comonentName, points) -> points.forEach((pointName, stats) -> stats.forEach((stat) -> strBld.append(comonentName+"."+pointName+"() \t: \t"+stat.executionTime()+"(ns)\t|\t"+stat.executionTime()/1000000+"(ms)\n"))));
        return strBld;
    }
}
